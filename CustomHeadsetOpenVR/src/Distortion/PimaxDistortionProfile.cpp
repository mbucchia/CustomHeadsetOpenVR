...
void PimaxDistortionProfile::Initialize() {
    pvr_getEyeDisplayInfo(PimaxCommon::GetPvrSession(), pvrEye_Left, &displayInfo);
...
Point2D PimaxDistortionProfile::ComputeDistortion(vr::EVREye eEye, ColorChannel colorChannel, float fU, float fV) {
    ...
    const float minResolution = (float)std::min(displayInfo.width, displayInfo.height);
    fU = (fU * minResolution / (2.0f * displayInfo.height)) + 0.5f;
    fV = (fV * minResolution / (2.0f * displayInfo.width)) + 0.5f;
