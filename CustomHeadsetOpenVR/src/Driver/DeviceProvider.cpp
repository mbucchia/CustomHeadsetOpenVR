...
vr::EVRInitError CustomHeadsetDeviceProvider::Init(vr::IVRDriverContext *pDriverContext){
	...
	if(driverConfig.dreamAir.enable && PimaxCommon::GetHeadsetInfo().connected && PimaxCommon::GetHeadsetInfo().isOpenPortEnabled &&
		PimaxCommon::GetHeadsetInfo().useSlamTracking) {
