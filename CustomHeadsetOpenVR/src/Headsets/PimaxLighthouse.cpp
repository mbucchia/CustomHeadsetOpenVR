...
bool PimaxLighthouseShim::IsDesiredHeadset(std::string model, vr::PropertyContainerHandle_t container){
	...
	if(trackingSystem == "lighthouse"){
		if(GetHeadsetInfo().headsetType == DreamAir && model == "Pimax Dream Air") {
			return true;
		}
	}
	return false;
...
Config::BaseHeadsetConfig& PimaxLighthouseShim::GetConfig() {
	switch (GetHeadsetInfo().headsetType) {
	case DreamAir:
	default:
		return PatchDisplayConfig(driverConfig.dreamAir);
	}
}
...
Config::BaseHeadsetConfig& PimaxLighthouseShim::GetConfigOld() {
	...
	case DreamAir:
	default:
		return PatchDisplayConfig(driverConfigOld.dreamAir);
	}
}
